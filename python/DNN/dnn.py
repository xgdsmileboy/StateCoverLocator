...
from DNN import var_model
from DNN import expr_model
...
class DNN(object):
    ...
    def train(self, X, Y, feature_num, class_num, model_dir_, is_var):
        X_train, X_valid, y_train, y_valid = train_test_split(X, Y, test_size = 0.25, random_state = 7)

        if is_var:
        	default_model = '/tmp/tmp_dnn_var'
        else:
        	default_model = '/tmp/tmp_dnn_expr'
        if os.path.exists(default_model):
            su.rmtree(default_model)

        if is_var:
            classifier = var_model.get_dnn_classifier(feature_num, class_num, default_model)
        else:
            classifier = expr_model.get_dnn_classifier(feature_num, class_num, default_model)
        ...
        if is_var:
            ...
            classifier = expr_model.get_dnn_classifier(feature_num, class_num, model_dir_)

        print(classifier.evaluate(input_fn=valid_input_fn))

        return classifier

    def evaluate(self, X, Y, feature_num, class_num, is_var):
        X_train, X_test, y_train, y_test = train_test_split(X, Y, test_size=0.2, random_state = 7)
        if is_var:
        	default_eval_model = '/tmp/tmp_dnn_var_val'
        else:
        	default_eval_model = '/tmp/tmp_dnn_expr_eval'
        classifier = self.train(X_train, y_train, feature_num, class_num, default_eval_model, is_var)
